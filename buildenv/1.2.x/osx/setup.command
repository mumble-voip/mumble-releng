...
if [ ! -d "/MumbleBuild" ]; then
	...
	echo
	echo "The Mumble 'osx' build environment stores its installed"
	...
	echo "   sudo chown \$USER:admin /MumbleBuild"
	exit 1
fi
...
if [[ $* == *--non-interactive* ]]; then
	...
	echo
	echo "Opening the directory in the Finder..."
	open -a Finder "${BUILDENV_TARGET}"
fi