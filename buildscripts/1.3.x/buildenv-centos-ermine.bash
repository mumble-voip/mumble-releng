...
cd buildenv/1.3.x/centos-ermine
BUILDENV_DIR=$(./setup.bash --non-interactive)
...
cd ${MUMBLE_PREFIX}/mumble-releng/buildenv/1.3.x/centos-ermine
./build-all.bash
...
# the old build env.
if [ -L /MumbleBuild/latest-1.3.x ]; then
    # First, read the link.
    OLD_BUILDENV_DIR=$(readlink /MumbleBuild/latest-1.3.x)
    OLD_BUILDENV_BUILD_DIR="${OLD_BUILDENV_DIR}.build"
    rm -rf "${OLD_BUILDENV_DIR}"
    rm -rf "${OLD_BUILDENV_BUILD_DIR}"
    unlink /MumbleBuild/latest-1.3.x
fi
ln -sf ${BUILDENV_DIR} /MumbleBuild/latest-1.3.x
